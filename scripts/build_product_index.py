#!/usr/bin/env python3
"""
Build a compact JSON image index for the PT Inventory Analyzer.

Output: download/product_index.json
{
  "byCode": {"MGE30003": {"num":"1212", "desc":"...", "img":"1212.jpg"}},
  "byBarcode": {"4214000006900": {...}},
...
def main():
    ...
    with open(PRODUCTS_CSV, encoding="utf-8", newline="") as f:
        for r in csv.DictReader(f):
            ...
            entry = {"num": num, "desc": desc[:200], "img": img}
            if code:
                by_code[code] = entry
            if barcode:
                by_barcode[barcode] = entry
            if desc:
                by_desc.append({"num": num, "desc": desc[:120].upper(), "img": img})
